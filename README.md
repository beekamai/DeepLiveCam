...
- **Black picture and 1 fps right after Live** — the chosen device is a virtual camera *output* (OBS Virtual Camera can be registered under any name, even a webcam's); the list labels it "virtual camera (OBS)", puts it last and refuses to open it (the OBS filter crashes the process when opened with webcam parameters) — pick the real webcam. The filter is recognised by its CLSID, so the label does not depend on its name. A real camera that starts black is reopened once automatically.
- **Virtual camera shows black in Discord / Zoom** — OBS 32's virtual camera driver does not render frames sent by pyvirtualcam (seen on 32.2.2). Install [Unity Capture](https://github.com/schellingb/UnityCapture) (its `Install.bat`); the app prefers it automatically.
...
- **Чёрная картинка и 1 fps сразу после Live** — выбранное устройство это *выход* виртуальной камеры (OBS Virtual Camera может быть зарегистрирована под любым именем, даже именем веб-камеры); список помечает её как «виртуальная камера (OBS)», ставит последней и отказывается открывать (фильтр OBS роняет процесс при открытии с параметрами веб-камеры) — выберите настоящую веб-камеру. Фильтр опознаётся по CLSID, от имени пометка не зависит. Настоящая камера, стартовавшая чёрной, один раз переоткрывается сама.
- **Виртуальная камера чёрная в Discord / Zoom** — драйвер виртуальной камеры OBS 32 не отрисовывает кадры от pyvirtualcam (проверено на 32.2.2). Поставьте [Unity Capture](https://github.com/schellingb/UnityCapture) (его `Install.bat`); приложение само предпочтёт его.
