...
- **Black picture and 1 fps right after Live** — the chosen device is a virtual camera *output* (OBS Virtual Camera can be registered under any name, even a webcam's); the list now labels it and puts it last — pick the real webcam. A camera that starts black is reopened once automatically.
...
- **Чёрная картинка и 1 fps сразу после Live** — выбранное устройство это *выход* виртуальной камеры (OBS Virtual Camera может быть зарегистрирована под любым именем, даже именем веб-камеры); список теперь помечает её и ставит последней — выберите настоящую веб-камеру. Камера, стартовавшая чёрной, один раз переоткрывается сама.
