...
if sys.platform == "win32":
    ...
    # Load cublas/cudnn from the nvidia-* wheels before any InferenceSession.
    # onnxruntime-gpu 1.26 lists CUDAExecutionProvider even when those DLLs
    # are missing; without preload it silently falls back to CPU.
    try:
        from onnxruntime import preload_dlls
        preload_dlls()
    except Exception as exc:
        print(f"[startup] CUDA DLL preload skipped: {exc}", flush=True)
