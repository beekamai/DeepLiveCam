...
from modules.swapper_registry import KEYS as SWAPPER_KEYS, DEFAULT_KEY as DEFAULT_SWAPPER_KEY
...
def parse_args() -> None:
    ...
    program.add_argument('--frame-processor', help='pipeline of frame processors', dest='frame_processor', default=['face_swapper'], choices=['face_swapper'] + ENHANCER_KEYS, nargs='+')
    ...
    if args.calibration_profile:
        ...
        if calibration.activate_saved(args.calibration_profile) is None:
            print(f"[calibration] profile not found: {args.calibration_profile}")
    modules.globals.face_tracking = not args.no_face_tracking
