...
def _enable_det_cuda_graph(fa: Any, providers) -> None:
    """Swap the detector session for a CUDA-graph one at the fixed det_size.

    det_10g is launch-bound rather than compute-bound — its runtime barely
    moves between a 256 and a 640 input — so replaying a recorded graph is
    where the time comes back: 12 ms -> 6 ms per frame at 640x640.
    """
    if not any(
    ...
    det_model = getattr(fa, "det_model", None)
    model_path = getattr(det_model, "model_file", None)
    if det_model is None or not model_path or not os.path.exists(model_path):
        return

    from modules.cuda_graph import GraphSession, build_static_model
    from modules.paths import MODELS_DIR

    width, height = det_model.input_size
    static_path = build_static_model(
        model_path,
        os.path.join(MODELS_DIR, f"det_10g_{width}x{height}.onnx"),
        {0: 1, 2: height, 3: width},
    )
    if static_path is None:
        return

    try:
        det_model.session = GraphSession(
            static_path, det_model.input_name, (1, 3, height, width),
        )
        print(f"[DLC.FACE-ANALYSER] detector CUDA graph enabled ({width}x{height})")
    except Exception as error:
        print(f"[DLC.FACE-ANALYSER] detector CUDA graph unavailable: {error}")

    # The 106-point landmark model is even smaller and pays the same launch
    # overhead — it runs once per frame whenever a landmark-shaped mask is on.
    landmark_model = fa.models.get("landmark_2d_106")
    landmark_path = getattr(landmark_model, "model_file", None)
    if landmark_model is None or not landmark_path or not os.path.exists(landmark_path):
        return

    lm_width, lm_height = landmark_model.input_size
    lm_static = build_static_model(
        landmark_path,
        os.path.join(MODELS_DIR, f"2d106det_{lm_width}x{lm_height}.onnx"),
        {0: 1},
    )
    if lm_static is None:
        return
    try:
        landmark_model.session = GraphSession(
            lm_static, landmark_model.input_name, (1, 3, lm_height, lm_width),
        )
        print("[DLC.FACE-ANALYSER] landmark CUDA graph enabled")
    except Exception as error:
        print(f"[DLC.FACE-ANALYSER] landmark CUDA graph unavailable: {error}")
...
def _needs_landmark() -> bool:
    ...
    if getattr(modules.globals, "face_outline_mask", False):
        return True
    if getattr(modules.globals, "blink_reveal", True) or getattr(modules.globals, "eye_reveal", 0.0) > 0:
        return True
